...
Run the complete M4.2R local gate:
...
Press `Ctrl+C` to stop the preview. The historical M4.2 gate remains immutable. The revised gate status is `pending_human_replaytest`; automation cannot claim that the failed human dimensions are resolved.

The empirical chain is first-class and machine-bound:

`M4.2 machine QA PASS → Human Playtest request_revision → M4.2R Revision Contract → revised Source Manifest/QA → Human Re-Playtest Gate`
...
M3 automation stops at `pending_human_selection`. M4.1 binds the selected concept to the smallest playable slice. M4.2 machine validation did not override a failed Human Playtest; M4.2R performs only the authorized bounded revision and stops at `pending_human_replaytest`. Complete-game scope, creative approval, M4.3, model APIs, Figma, deployment, backend services, accounts, analytics, and CI remain deferred.
