...
|---|---|---|---|---|
| [Playwright](https://github.com/microsoft/playwright) | Isolated explicit viewport contexts, screenshots, request routing, DOM evaluation, and bounded `DOMContentLoaded`-based observation. | `networkidle` as universal readiness, its monorepo/browser patches, or unnecessary runner layers. | The used npm packages declare Apache-2.0; the upstream root LICENSE is currently absent. | `core` |
