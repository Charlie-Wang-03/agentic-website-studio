...
M4.2 keeps the Wayfinder dogfood isolated under `pilots/wayfinder/`. Its browser UI dispatches events to one pure transition layer; contract-conformance tests compare that layer with the M4.1 JSON state machine. Vite is build/preview infrastructure only. Playwright operates on the local production build and checks the branch matrix, keyboard, emulated touch, reduced motion, representative axe states, network isolation, runtime errors, and deterministic replay. Wayfinder is Pilot 1 and is now a completed internal dogfood pilot; its prior pending M4.3R Human Expansion Re-Playtest Gate remains historically unanswered. The separate Pilot Closure artifact does not rewrite that gate as approval, and no release or deployment is authorized.
...
M3 ends at a first-class Human Creative Gate whose automation schema can emit only `pending_human_selection`. M4.1 records the human decision and approved slice. M4.2 stops at `pending_human_playtest`; a human may reject it despite green machine QA. M4.2R accepts only a bound `request_revision`, preserves the failed experiment, and stops at `pending_human_replaytest`. M4.3R similarly preserves an unanswered `pending_human_expansion_replaytest` gate. The later `close_pilot` decision ends the Wayfinder pilot as internal dogfood only; it is not release approval or evidence that its unanswered questions passed. Candidate Rules extracted in [Wayfinder Project Lessons](WAYFINDER_PROJECT_LESSONS.md) require cross-pilot or equivalent evidence before promotion. Evidence before self-improvement.
